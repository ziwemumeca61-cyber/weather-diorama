...
3. 正式端点击“制作城市画报”只下载现成素材；雨、雪、雾、雷、阴云和晴光由 `weatherMoodSticker.js` 在本地 Canvas 绘制，文字也只在本地叠加。
...
- **发布动态**：第一栏只保留公共“发一条”和 `official-account-publish` 话题组件。照片选择、拍照、文字编辑和发布都在微信官方发表页完成；组件继续使用“天气心情贴”话题，以展示历史已发布内容。
- **城市画报**：第二栏按“选择心情 → 选择画风 → 填写可选文案 → 制作 → 预览 → 保存或发布”的顺序操作。正式端只读取预生成城市画面，再在本地叠加实时天气与文字，不调用模型。
...
- **生成内容标识**：城市画报成品右上角稳定显示“AI 预生成素材”；发布正文不再追加技术说明。
- **本地缓存**：客户端仅缓存云素材的 `fileID`；切换城市、心情或画风时读取对应素材，不产生生图请求。
- **发布隔离**：公共“发一条”不会附带上一次生成的城市画报；只有画报完成后的“发布城市画报”会携带合成图片和画报文案进入官方发表页。

...
- **我的图片**：点击“选择图片”从相册或相机选择并在本机合成，完成后使用该页独立的“发一条 · 我的图片”。
- **风格素材**：选择心情和五种画风之一，点击“制作风格天气心情贴”，完成后使用该页独立的“发一条 · 风格素材”。正式端只读取预生成云素材，不调用模型。
...
- **生成内容标识**：风格素材成品右上角稳定显示“AI 预生成素材”；发布正文不再追加技术说明。
- **本地缓存**：客户端仅缓存云素材的 `fileID`；切换城市、心情或风格时读取对应素材，不产生生图请求。
- **直接发表贴图**：我的图片与风格素材各有独立的 `wx.shareToOfficialAccount` 入口；底部同时保留公共“发一条”和 `official-account-publish` 话题组件。公共入口在没有现成贴图时也能进入官方发表页，组件用于显示已经发布的“天气心情贴”内容。

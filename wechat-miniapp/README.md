...
- **AI 心情贴**：制作页顶部固定显示「✨ AI 生图」切换入口；选择心情和风格后，直接点击风格区下方的「生成 AI 天气心情贴」。用户只能选择预设心情与预设风格，不能向生图模型提交自由提示词。`moodSticker` 调用成长计划的 `HY-Image-3.0-Plus-4090-Tob-v1.0`，同时约束真实城市/区县、城市地标、当前天气、所选心情和画风。现有五种画风包含电影叙事、3D微缩、治愈插画、东方留白和城市采集志。用户写的心情文字不发给模型，而是在本机与默认心情文案、城市、天气一起合成为准确中文海报。
- **额度保护**：客户端缓存相同“城市＋日期＋天气＋心情＋风格”的背景 7 天；云函数实例内也会跨用户复用相同背景。命中缓存时不调用生图。单用户 10 分钟最多新生成 2 次，失败后不自动重试。修改心情文字只本地重新合成，不重新生图。
- **城市采集志**：用一个主场景和 3–5 个同城碎片组织 zine 式叙事，包含地标切片、街角物件、天气痕迹和纸张/胶带/网点肌理，明确禁止九宫格、PPT拼版和随机城市素材。

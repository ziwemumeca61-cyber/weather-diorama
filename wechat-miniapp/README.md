...
3. 给 `moodAssetBatch` 配置环境变量 `MOOD_ASSET_ADMIN_SECRET`，值使用你自己生成的长随机字符串；函数超时设置为 **180 秒**。
4. 右键 `cloudfunctions/moodAssetBatch` → “上传并部署（云端安装依赖）”，再单独上传 `config.json` 的定时触发器。触发器每两分钟运行一次，每次最多处理 6 张、并发 2 张；重复任务会先查数据库，已成功的素材不会再次生图。
5. 在云函数测试页启动任务：
...
当 `job.status` 变成 `complete` 且 `nextIndex` 为 `2226` 时生成完成。理论最短约 12.5 小时，实际会受模型耗时和限流影响；失败批次保持原游标，由下一次定时任务幂等重试。

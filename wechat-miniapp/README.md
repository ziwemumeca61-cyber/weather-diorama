...
- **我的照片**：空话题点击「发一条」后进入微信官方发表页选图；已经生成的心情贴会优先由官方发表 API 自动带入。
...
- **话题内容区**：底部的官方 `official-account-publish` 组件固定使用「天气心情贴」话题，用于浏览同话题内容；空话题时由 `wx.shareToOfficialAccount` 提供同名「发一条」入口，有内容时使用组件内置入口。
